...
static void perform_click(int button);
...
static void handle_key_event(XKeyEvent *e) {
	...
	switch(key) {
		...
		case KEY_DOCK_TOGGLE:
			...
			break;
        // lauchimoon 10/10/23:
        case KEY_MV_MOUSE_UP: {
                int src_x = 0;
                int src_y = 0;
                get_mouse_position(&src_x, &src_y, e->root);

                int dst_x = src_x;
                int dst_y = src_y - offset;
                setmouse(e->root, dst_x, dst_y);

                XFlush(dpy);
            }
            break;
        case KEY_MV_MOUSE_LEFT: {
                int src_x = 0;
                int src_y = 0;
                get_mouse_position(&src_x, &src_y, e->root);

                int dst_x = src_x - offset;
                int dst_y = src_y;
                setmouse(e->root, dst_x, dst_y);
        ...
        case KEY_MV_MOUSE_RIGHT: {
            ...
            break;
        case KEY_CLICK_MOUSE_L:
#define MOUSE_LEFT_BUTTON 1
            perform_click(MOUSE_LEFT_BUTTON);
            break;
        case KEY_CLICK_MOUSE_R:
#define MOUSE_RIGHT_BUTTON 3
            perform_click(MOUSE_RIGHT_BUTTON);
            break;
#ifdef VWM
